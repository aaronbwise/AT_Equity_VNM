...
from tabulate import read_csv_file
...
def create_excl_bf(df, country, year):
    ...
    liquids_no_values = config_data[country][year]["excl_bf"]["liquids"]["values"][0]
    solids_no_values = config_data[country][year]["excl_bf"]["solids"]["values"][0]

    ## Create sub-indicators

    ## No liquids sub-indicator
    df['liquids_none'] = np.where(df[var_liquids].eq(liquids_no_values).all(axis = 1), 100, 0)

    ## No solids sub-indicator
    df['solids_none'] = np.where(df[var_solids].eq(solids_no_values).all(axis = 1), 100, 0)

    # Create indicator
    df['excl_bf'] = np.where((df[var_breastmilk] == breastmilk_yes_values) & (df['liquids_none'] == 100) & (df['solids_none'] == 100), 100, 0)
...
def create_cont_1223_bf(df, country, year):
    ...
    # Create indicator
    df['cont_1223_bf'] = np.where(df[var_breastmilk] == breastmilk_yes_values, 100, 0)
...
def create_mdd_ch(df, country, year):
    ...
    """
    # :: COL_NAMES
    var_breastmilk = config_data[country][year]["excl_bf"]["breastmilk"]["col_names"]
    ...
    ## Create sub-indicators
    df['breastmilk'] = np.where(df[var_breastmilk].eq(breastmilk_yes_values).any(axis = 1), 100, 0)
...
def create_mmf_ch(df, country, year):
    ...
    df['mmf_bf_68'] = np.where((df[var_breastmilk] == breastmilk_yes_values) & (df['solid_semi_soft_times'] >= 2), 100, 0)
    df['mmf_bf_68'] = np.where(df['ch_age_cat_6_8'] == 0, 0, df['mmf_bf_68'])

    ## BREASTFED: Age 9-23 months with 3 soft, semi-soft, solid feeds
    df['mmf_bf_923'] = np.where((df[var_breastmilk] == breastmilk_yes_values) & (df['solid_semi_soft_times'] >= 3), 100, 0)
