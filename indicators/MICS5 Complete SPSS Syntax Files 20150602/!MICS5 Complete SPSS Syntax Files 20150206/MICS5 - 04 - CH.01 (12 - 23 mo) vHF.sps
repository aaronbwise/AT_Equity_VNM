...
define before12 (v_12 = !tokens(1) / vacc = !tokens(1) / hfd = !tokens(1) / hfm = !tokens(1) / hfy = !tokens(1) / vd = !tokens(1) / vm = !tokens(1) / vy = !tokens(1) / vlabel = !tokens(1)).
...
*Uncomment if measels given at age 12 mo or later.
* define before24 (v_24 = !tokens(1) / vacc = !tokens(1) / hfd = !tokens(1) / hfm = !tokens(1) / hfy = !tokens(1) / vd = !tokens(1) / vm = !tokens(1) / vy = !tokens(1) / vlabel = !tokens(1)).

* Calculate whether child who received vaccination according to a vaccination card
* received it before her 2st birthday.
...
do if (vacc = 1 or vacc = 2 or vacc = 3).
* as mmr is administered at age of 12 month set it to system missing.
*+ compute basicvacc = basicvacc*lag(basicvacc)/100.
*+ compute novacc = 100-(100-novacc)*(100-lag(novacc))/100.
+ compute basicvacc = $sysmis.
end if.
