...
get file = "hl.sav".
fre vars = hl3 hl12a hl14a.

* Merge servant and abroad variables from hl onto hh.
recode hl3 (14=1) (else=0) into servant.
compute abroad = 0.
if (hl12a = 3 or hl14a = 3) abroad = 1.
execute.

sort cases hh1 hh2.
aggregate /outfile="temp.sav" /break=hh1 hh2
	         /servant	"One or more live-in servants" = max(servant) 
	         /abroad	"One or more parent abroad" = max(abroad).
...
ctables 	/table ( !1 ) [s] [mean '' f5.4] 
	by nini1 [c] + ncom1 [c]
	/categories variables = ncom1 total = yes
	/titles title=Surveyname
